...
const features = data
  .map((item, index) => {
    ...
    // 處理座標（逗號分隔）
    const xCoords = item["路徑（X）"]
      .split(",")
      .map((s) => parseFloat(s.trim()))
      .filter((n) => !isNaN(n) && isFinite(n));
    const yCoords = item["路徑（Y）"]
      .split(",")
      .map((s) => parseFloat(s.trim()))
      .filter((n) => !isNaN(n) && isFinite(n));
    ...
    if (xCoords.length !== yCoords.length) {
      console.warn(
        `路段 ${item["路段序號"]} X 和 Y 座標數量不一致 (X:${xCoords.length}, Y:${yCoords.length})，已跳過`,
      );
      return null;
    }

    // TWD97 -> WGS84 經緯度轉換
    const coordinates = xCoords
      .map((x, i) => {
        const y = yCoords[i];
        try {
